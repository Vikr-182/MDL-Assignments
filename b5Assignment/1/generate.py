                    ...
                    for ag_un in range((2)):# Agent may or may not acheive the desired outcome
                        for call_un in range((2)):# Call may or may not acheive the desired outcome
                            ...
                            if agent_state == target_state and call_state == 1: #agent and target are in same place and call is on
                                call_state = 0
                                call_prob = 1#0.4*( 1)*(call_un) + 0.6*( 1)*(1  - call_un)
                                if call_un is 0:
                                    continue
                                else:
                                    j = get_overall_num(take_action(agent_state[0],agent_state[1],actions[ag_ac],ag_un),take_action(target_state[0],target_state[1],actions[tar_ac],1) ,1)#cn(call_state,call_un))
